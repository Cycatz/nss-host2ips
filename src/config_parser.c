...
{
    my_malloc(*host_list, sizeof(HOST_LIST), 0);
...
{
    my_malloc(host->name, strlen(line) + 1 - 5, 0);
...
int parse_host_info(char *info, HOST_INFO *host_info)
{
    ...
    if_name = strtok(NULL, " \t");
    my_malloc(host_info->if_name, strlen(if_name) + 1, 0);
...
int parse_config_file(const char *path, HOST_LIST *host_list)
{
    ...
    while (fgets(line, MAX_LINE_SIZE - 1, fp)) {
        if (!is_valid_line(line)) {
            ...
            if (strncmp(line, "host", 4) == 0) {
                ...
                my_malloc(host, sizeof(HOST), 0);
                parse_host_name(line, host);
            } else {
                my_malloc(host_info, sizeof(HOST_INFO), 0);
    ...
    if (!add_new_host(host_list, host)) {
        return 0;
    }
